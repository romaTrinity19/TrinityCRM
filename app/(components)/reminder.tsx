...
function ReminderScreen() {
  ...
  const renderMenu = (item: any) => (
    <Modal
      transparent={true}
      visible={selectedItem === item.reminder_id && menuVisible}
      animationType="fade"
          ...
          {[
            {
              ...
              onPress: () => {
                ...
                router.push({
                  pathname: "/(components)/reminderDetails",
                  params: { id: item.reminder_id },
                });
              },
            },
            {
              ...
              onPress: () => {
                setMenuVisible(false);
                router.push({
                  pathname: "/(components)/createReminder",
                  params: { type: "update", id: item.reminder_id },
                });
              },
            },
            {
            ...
            },
            {
              ...
              onPress: () => {
                setMenuVisible(false);
                setFollowUpToDelete(item.reminder_id);
  ...
  const fetchFollowUps = async () => {
    try {
      ...
      const response = await fetch(
        "http://crmclient.trinitysoftwares.in/crmAppApi/reminder.php?type=getAllReminders"
      );
  ...
  const handleConfirmDelete = async (reminderId: string) => {
    try {
      const response = await fetch(
        "http://crmclient.trinitysoftwares.in/crmAppApi/reminder.php?type=deleteReminder",
        {
          method: "DELETE",
          headers: {
  ...
  const applyFilter = async () => {
    try {
      ...
      const response = await axios.post(
        "http://crmclient.trinitysoftwares.in/crmAppApi/reminder.php?type=getFilteredReminders",
        payload,
        {
  ...
  return (
    <View style={styles.container}>
            ...
            <Card style={styles.card}>
              <TouchableOpacity>
                <View style={styles.cardHeader}>
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      setSelectedItem(item.reminder_id);
                      setMenuVisible(true);
                    }}
                  >
                ...
                <View style={styles.cardDetailRow}>
                  ...
                  <Text style={styles.cardValue}>{item.reminder_date}</Text>
                </View>

                <View style={styles.cardDetailRow}>
                  <Ionicons name="time-outline" size={16} color="#4B65E9" />
                  <Text style={styles.cardLabel}>Reminder Time</Text>
                  <Text style={styles.cardValue}>{item.reminder_time}</Text>
                </View>
                <View style={styles.cardDetailRow}>
                  <Ionicons
      ...
      >
        <Text style={styles.createButtonText}>Reminder</Text>
      </TouchableOpacity>
    </View>
  );
}
